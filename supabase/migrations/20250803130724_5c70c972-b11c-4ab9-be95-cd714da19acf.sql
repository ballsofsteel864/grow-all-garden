-- Fix restock system and RLS policies
...
-- 2. Update all existing shop stock with proper restock chances and times
UPDATE shop_stock 
SET 
  ...
  END,
  current_stock = CASE 
    WHEN s.rarity = 'common' THEN max_stock
    WHEN s.rarity = 'uncommon' AND random() < 0.7 THEN max_stock
    WHEN s.rarity = 'rare' AND random() < 0.3 THEN max_stock
    WHEN s.rarity = 'epic' AND random() < 0.1 THEN max_stock
    WHEN s.rarity = 'legendary' AND random() < 0.05 THEN max_stock
    WHEN s.rarity = 'mythic' AND random() < 0.01 THEN max_stock
    ELSE 0
  END
FROM seeds s 
